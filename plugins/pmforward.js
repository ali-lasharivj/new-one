...
cmd({
  ...
  try {
    // Owner check
    if (!isOwner) return await message.reply("*📛 ᴛʜɪs ɪs ᴀɴ ᴏᴡɴᴇʀ ᴄᴏᴍᴍᴀɴᴅ.*");
    
    // Quoted message check
    if (!message.quoted) return await message.reply("*🛰️ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ғᴏʀᴡᴀʀᴅ*");
    ...
    if (validJids.length === 0) {
      return await message.reply(
        ...
        "*ғᴏʀ ᴘʀɪᴠᴀᴛᴇ ɴᴜᴍʙᴇʀ: 92300357379*"
      );
    }
    ...
    let report = `*✅ ғᴏʀᴡᴀʀᴅ ᴄᴏᴍᴘʟᴇᴛᴇᴅ*\n\n` +
                 `*🪀 sᴜᴄᴄᴇss:* ${successCount}/${validJids.length}\n` +
                 `*🏮 ᴍᴇᴅɪᴀ ᴛʏᴘᴇ:* ${mtype.replace('Message', '') || 'text'}\n`;

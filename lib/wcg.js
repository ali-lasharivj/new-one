...
class Game {
  ...
  async validateTurn(playerId, word) {
    ...
    if (word.length < this.minLength) return { valid: false, reason: `*_Word must be at least ${this.minLength} letters_*` };
    ...
    if (this.lastWord && word[0] !== this.lastWord[this.lastWord.length - 1]) {
      return { valid: false, reason: `*_Word must start with '${this.lastWord[this.lastWord.length - 1]}'_*` };
    }
